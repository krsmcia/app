<div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            ...
            @foreach ($requests as $request)
                ...
                >
                    ...
                    {{-- =====================================================
                        Transaction History
                    ====================================================== --}}
                    @if ($request->purchaseRequestTransactions->isNotEmpty())
                        <div class="border-b border-gray-100 bg-gray-50/50 px-4 py-3 sm:px-5">
                            ...
                            <div class="mb-2 flex items-center justify-between">
                                ...
                                <span class="text-[10px] text-gray-400">
                                    {{ $request->purchaseRequestTransactions->count() }}
                                    {{ $request->purchaseRequestTransactions->count() === 1 ? 'transaction' : 'transactions' }}
                                </span>
                            </div>

                            <div class="overflow-hidden rounded-lg border border-gray-200 bg-white">
                                @foreach ($request->purchaseRequestTransactions as $purchaseRequestTransaction)
                                    <div
                                        wire:key="request-transaction-{{ $purchaseRequestTransaction->transaction->id }}"
                                        class="flex items-center gap-2.5 border-b border-gray-100 px-3 py-2.5 last:border-b-0 sm:px-4"
                                    >

                                        <div class="flex shrink-0 items-center">
                                            <img
                                                src="{{ $purchaseRequestTransaction->transaction->fromUser->profile_photo_url }}"
                                                alt="{{ $purchaseRequestTransaction->transaction->fromUser->name }}"
                                                class="size-7 rounded-full object-cover"
                                            >

                                            <span class="mx-1 text-[10px] text-gray-300">
                                                →
                                            </span>

                                            <img
                                                src="{{ $purchaseRequestTransaction->transaction->toUser->profile_photo_url }}"
                                                alt="{{ $purchaseRequestTransaction->transaction->toUser->name }}"
                                                class="size-7 rounded-full object-cover"
                                            >
                                        </div>
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <div class="flex min-w-0 items-center gap-1 text-xs font-medium text-gray-800">
                                                <span class="truncate">
                                                    {{ $purchaseRequestTransaction->transaction->fromUser->name }}
                                                </span>
                                                ...
                                                <span class="truncate">
                                                    {{ $purchaseRequestTransaction->transaction->toUser->name }}
                                                </span>
                                            </div>

                                            <div class="mt-0.5 flex items-center gap-1.5 text-[9px] text-gray-400 sm:text-[10px]">
                                                <span class="uppercase">
                                                    {{ $purchaseRequestTransaction->transaction->type }}
                                                </span>

                                                @if ($purchaseRequestTransaction->transaction->created_at)
                                                    <span>·</span>

                                                    <span>
                                                        {{ $purchaseRequestTransaction->transaction->created_at->format('M d, Y H:i') }}
                                                    </span>
                                                @endif
                                            </div>
                                        ...
                                        <div class="shrink-0 text-right">
                                            ...
                                            <div class="text-xs font-bold text-gray-900 sm:text-sm">
                                                ₱{{ number_format($purchaseRequestTransaction->transaction->amount, 2) }}
                                            </div>

                                            @if ($purchaseRequestTransaction->transaction->type === 'released')
                                                <div class="text-[9px] font-medium text-emerald-600">
                                                    Released
                                                </div>
                                            @endif
                    ...
                    <div class="divide-y divide-gray-100">
                        @foreach ($request->items as $workflowItem)
                            <div
                            ...
                            >
                                ...
                                <div class="hidden md:flex md:items-center md:gap-5">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            ...
                                            <button
                                                ...
                                                x-on:click="$dispatch('open-item', {
                                                    itemId: {{ $workflowItem->purchaseItem->item_id }}
                                                })"
                                                class="mt-0.5 block max-w-full truncate text-sm font-semibold text-gray-900 hover:text-indigo-600"
                                                wire:loading.attr="disabled"
                                            >
                                            ...
                                            @if ($workflowItem->purchaseItem->itemVendor)
                                                <button
                                                    type="button"
                                                    x-on:click="$dispatch('open-vendor', { 
                                                        vendorId: {{ $workflowItem->purchaseItem->itemVendor->vendor_id }} 
                                                    })"
                                                    class="mt-0.5 block max-w-[220px] truncate text-xs font-medium text-gray-700 hover:text-indigo-600"
                                                    wire:loading.attr="disabled"
                                                >
                                                    {{ $workflowItem->purchaseItem->vendor_name }}
                                                </button>
                                            @else
                                                <div class="mt-0.5 max-w-[220px] truncate text-xs text-gray-500">
                                                    {{ $workflowItem->purchaseItem->vendor_name ?: '-' }}
                                                </div>
                                            @endif
                                    ...
                                    {{-- Pricing Details --}}
                                    <div class="w-[260px] shrink-0 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2.5">
                                        ...
                                        <div class="grid grid-cols-2 gap-x-5 gap-y-2">
                                            ...
                                            <div>
                                                ...
                                                <div class="mt-0.5 text-xs font-semibold text-gray-800">
                                                    {{ number_format($workflowItem->purchaseItem->quantity) }}
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <div class="mt-0.5 text-xs font-semibold text-gray-800">
                                                    ₱{{ number_format($workflowItem->purchaseItem->shipping_fee, 2) }}
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <div class="mt-0.5 text-xs font-semibold text-gray-800">
                                                    −₱{{ number_format($workflowItem->purchaseItem->discount, 2) }}
                                                </div>
                                            </div>
                                    ...
                                    {{-- Release Amount --}}
                                    <div class="w-[190px] shrink-0 rounded-lg border border-emerald-100 bg-emerald-50 px-4 py-3 text-right">

                                        <div class="flex items-center justify-end gap-1.5">
                                            <span class="text-[9px] font-semibold uppercase tracking-wider text-emerald-600">
                                                Release
                                            </span>

                                            <span class="rounded-full bg-emerald-100 px-1.5 py-0.5 text-[9px] font-bold uppercase text-emerald-700">
                                                Cash
                                            </span>
                                        </div>

                                        <div class="mt-1 text-xl font-extrabold leading-tight tracking-tight text-emerald-900">
                                            ₱{{ number_format($workflowItem->release_total, 2) }}
                                        </div>

                                        <div class="mt-0.5 text-[9px] text-emerald-600">
                                            Calculated ₱{{ number_format($workflowItem->calculated_total, 2) }}
                                        </div>
                                    </div>
                                ...
                                <div class="md:hidden">
                                    ...
                                    <div class="flex items-start gap-3">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <button
                                                ...
                                                class="block w-full truncate text-left text-sm font-semibold text-gray-900 hover:text-indigo-600"
                                                wire:loading.attr="disabled"
                                            >
                                                {{ $workflowItem->purchaseItem->item_name }}
                                            </button>
                                    ...
                                    <div class="mt-3 rounded-lg border border-gray-100 bg-gray-50 p-3">
                                        ...
                                        @if ($workflowItem->is_money_holder)
                                            {{-- Release Amount --}}
                                            <div class="mt-4 rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-3">
                                                ...
                                                <div class="flex items-center justify-between gap-3">
                                                    ...
                                                    <div class="min-w-0">
                                                        ...
                                                        <div class="mt-1 flex items-center gap-1.5">
                                                            ...
                                                            <span class="text-[9px] text-emerald-600">
                                                                Calculated
                                                            </span>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="shrink-0 text-right">
                                                        <div class="whitespace-nowrap text-xl font-extrabold leading-tight tracking-tight text-emerald-900">
                                                            ₱{{ number_format($workflowItem->release_total, 2) }}
                                                        </div>

                                                        @if (isset($workflowItem->calculated_total))
                                                            <div class="mt-0.5 whitespace-nowrap text-[9px] text-emerald-600">
                                                                ₱{{ number_format($workflowItem->calculated_total, 2) }}
                                                            </div>
                                                        @endif
                                                    </div>
                                    ...
                                    <div class="mt-3">
                                        ...
                                        @if ($workflowItem->purchaseItem->itemVendor) 
                                            <button 
                                                ...
                                                class="mt-1 block max-w-full truncate text-left text-sm font-medium text-gray-700 hover:text-indigo-600" 
                                                wire:loading.attr="disabled" 
                                            > 
                                                {{ $workflowItem->purchaseItem->vendor_name }} 
                                            </button> 
                                    ...
                                    @if ($workflowItem->status === 'pending')
                                        ...
                                        <div class="mt-3">
                                            ...
                                            <div class="mt-1 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2.5 text-xs text-gray-600">
                                                <div class="break-words whitespace-pre-wrap leading-relaxed">
                                                    {{ $workflowItem->purchaseItem->payment_details ?: '-' }}
                                                </div>
                                            </div>
                                    ...
                                    @elseif ($workflowItem->status === 'ordered')
                                        ...
                                        <div class="mt-3">
                                            ...
                                            <div class="mt-1 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2.5 text-xs text-gray-600">
                                                <div class="break-words whitespace-pre-wrap leading-relaxed">
                                                    {{ $workflowItem->purchaseItem->remark ?: '-' }}
                                                </div>
                                            </div>
                                ...
                                <div class="mt-3 rounded-lg border border-indigo-100 bg-indigo-50 px-3 py-2.5">
                                    <div class="text-[10px] font-semibold uppercase tracking-wide text-indigo-500">
                                        Money Holder
                                    </div>

                                    @if ($workflowItem->money_holder)
                                        <div class="mt-1 flex items-center justify-between">
                                            <div class="flex items-center gap-2">
                                                <img
                                                    src="{{ $workflowItem->money_holder->profile_photo_url }}"
                                                    alt="{{ $workflowItem->money_holder->name }}"
                                                    class="size-7 rounded-full object-cover"
                                                >
                                                <div class="min-w-0">
                                                    <div class="truncate text-xs font-semibold text-gray-900">
                                                        {{ $workflowItem->money_holder->name }}
                                                    </div>
                                                    @if ($workflowItem->latest_money_transaction?->transaction)
                                                        <div class="mt-0.5 text-[9px] text-gray-400">
                                                            {{ ucfirst($workflowItem->latest_money_transaction->transaction->type) }}
                                                            ·
                                                            {{ $workflowItem->latest_money_transaction->transaction->created_at?->format('M d, Y H:i') }}
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
                                            @if (!$workflowItem->is_money_holder && $workflowItem->status === 'pending')
                                                <button
                                                    ...
                                                    class="inline-flex items-center gap-1.5 rounded-md bg-indigo-600 px-3 py-1.5 text-[10px] font-bold text-white shadow-sm transition hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1"
                                                >
                                                    <svg
                                                        class="size-3.5"
                                                        fill="none"
    ...
    <x-dialog-modal wire:model.live="commentModal">
        ...
        <x-slot name="footer">
            <div class="flex w-full justify-end gap-2">
                ...
                <x-approve-button
                    type="button"
                    wire:click="purchaseItem"
                    wire:loading.attr="disabled"
                    wire:target="purchaseItem"
                >
                    <span wire:loading.remove wire:target="purchaseItem">
                        {{ __('Complete Purchase') }}
                    </span>
                    <span wire:loading wire:target="purchaseItem">
                        {{ __('Processing...') }}
                    </span>
                </x-approve-button>
            </div>
    ...
    <livewire:procurements.modals.receive-cash />
</div>