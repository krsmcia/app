<div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    <div class="mb-6">
        <h1 class="text-xl font-semibold text-gray-900">
            Budget Requests
        </h1>
        <p class="mt-1 text-sm text-gray-500">
            Purchase requests waiting for budget processing.
        </p>
    </div>

    <div class="space-y-4">
        @if (count($requests) > 0)
            @foreach ($requests as $request)

                @php
                    /*
                     * Cash items are intentionally rendered after non-cash items.
                     * This keeps the budget-processing flow focused on actual
                     * payment/release actions first.
                     */
                    $workflowItems = $request->account_workflow
                        ->purchaseWorkflowItems
                        ->sortBy(function ($workflowItem) {
                            return strcasecmp(
                                trim($workflowItem->purchaseItem->disbursement_type_name ?? ''),
                                'cash'
                            ) === 0 ? 1 : 0;
                        })
                        ->values();

                    $hasCash = $workflowItems->contains(function ($workflowItem) {
                        return strcasecmp(
                            trim($workflowItem->purchaseItem->disbursement_type_name ?? ''),
                            'cash'
                        ) === 0;
                    });
                @endphp

                <div
                    class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm"
                    wire:key="request-{{ $request->id }}"
                >
                    {{-- =====================================================
                        Header
                    ====================================================== --}}
                    <div class="border-b border-gray-100 px-4 py-4 sm:px-5">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-2">
                                    <h2 class="text-sm font-semibold text-gray-900 sm:text-base">
                                        {{ $request->request_no }}
                                    </h2>

                                    <span class="rounded-full bg-yellow-50 px-2.5 py-1 text-[10px] font-medium text-yellow-700 sm:text-xs">
                                        Pending
                                    </span>
                                </div>

                                <div class="mt-1.5 text-xs text-gray-500 sm:flex sm:gap-2">
                                    <span>
                                        Requested by
                                    </span>

                                    <p
                                        class="font-medium text-gray-700"
                                        wire:loading.attr="disabled"
                                    >
                                        {{ $request->user->name }}

                                        @if ($request->department)
                                            <span class="mx-1 text-gray-300">
                                                ·
                                            </span>

                                            <span class="font-medium text-gray-700">
                                                {{ $request->department->name }}
                                            </span>
                                        @endif
                                    </p>
                                </div>
                            </div>

                            <div class="shrink-0 text-right text-[11px] text-gray-400 sm:text-sm sm:text-gray-500">
                                {{ $request->created_at->format('Y-m-d') }}

                                <div class="sm:hidden">
                                    {{ $request->created_at->format('H:i') }}
                                </div>

                                <span class="hidden sm:inline">
                                    {{ $request->created_at->format('H:i') }}
                                </span>
                            </div>
                        </div>
                    </div>

                    {{-- =====================================================
                        Items
                    ====================================================== --}}
                    <div class="divide-y divide-gray-100">

                        @foreach ($workflowItems as $workflowItem)

                            @php
                                $purchaseItem = $workflowItem->purchaseItem;
                                $item = $purchaseItem->item;
                                $itemVendor = $purchaseItem->itemVendor;

                                $isCash = strcasecmp(
                                    trim($purchaseItem->disbursement_type_name ?? ''),
                                    'cash'
                                ) === 0;
                            @endphp

                            <div
                                class="px-4 py-4 sm:px-5"
                                wire:key="workflow-item-{{ $workflowItem->id }}"
                                x-data="{
                                    unitPrice: {{ (float) $purchaseItem->unit_price }},
                                    quantity: {{ (float) $purchaseItem->quantity }},
                                    discount: {{ (float) ($purchaseItem->discount ?? 0) }},
                                    shipping: {{ (float) ($purchaseItem->shipping_fee ?? 0) }},

                                    get baseAmount() {
                                        return this.unitPrice * this.quantity;
                                    },

                                    get finalAmount() {
                                        return this.baseAmount - this.discount + this.shipping;
                                    },

                                    money(value) {
                                        return Number(value || 0).toLocaleString('en-PH', {
                                            minimumFractionDigits: 2,
                                            maximumFractionDigits: 2
                                        });
                                    }
                                }"
                            >

                                {{-- =================================================
                                    DESKTOP
                                ================================================== --}}
                                <div class="hidden items-center justify-between gap-4 md:flex">

                                    <div class="flex min-w-0 flex-1 items-center gap-4">

                                        {{-- Image --}}
                                        <div class="h-14 w-14 shrink-0 overflow-hidden rounded-md bg-gray-100">
                                            <img
                                                src="{{ $item?->primaryImage
                                                    ? Storage::url($item->primaryImage->path)
                                                    : asset('images/default-item.png') }}"
                                                alt="{{ $purchaseItem->item_name }}"
                                                class="h-full w-full object-cover"
                                            >
                                        </div>

                                        {{-- Item Info --}}
                                        <div class="min-w-0 flex-1">
                                            <div class="text-[10px] uppercase tracking-wide text-gray-400">
                                                Name
                                            </div>

                                            <span class="block max-w-full truncate text-sm font-medium text-gray-900">
                                                {{ $purchaseItem->item_name }}
                                            </span>

                                            {{-- Vendor --}}
                                            <div class="mt-1 w-52 min-w-0">
                                                <div class="text-[10px] uppercase tracking-wide text-gray-400">
                                                    Vendor
                                                </div>

                                                @if ($itemVendor)
                                                    <span class="mt-0.5 block w-full truncate text-left text-sm font-medium text-gray-700">
                                                        {{ $purchaseItem->vendor_name }}
                                                    </span>
                                                @else
                                                    <div class="mt-0.5 text-sm text-gray-500">
                                                        {{ $purchaseItem->vendor_name ?: '-' }}
                                                    </div>
                                                @endif
                                            </div>

                                            <div class="mt-1 flex items-center gap-3 text-xs text-gray-500">
                                                <span>
                                                    {{ $purchaseItem->sku }}
                                                </span>

                                                <span class="text-gray-300">|</span>

                                                <span>
                                                    Qty:
                                                    <span class="font-semibold text-gray-700">
                                                        {{ $purchaseItem->quantity }}
                                                    </span>
                                                </span>

                                                <span class="text-gray-300">|</span>

                                                <span>
                                                    Unit Price:
                                                    <span class="font-semibold text-gray-900">
                                                        {{ number_format($purchaseItem->unit_price, 2) }}
                                                    </span>
                                                </span>
                                            </div>
                                        </div>

                                        {{-- Adjustments --}}
                                        <div class="w-28 shrink-0 text-right">
                                            <div class="text-[10px] uppercase tracking-wide text-gray-400">
                                                Adjustments
                                            </div>

                                            <div class="mt-0.5 space-y-0.5 text-xs whitespace-nowrap">
                                                <template x-if="discount > 0">
                                                    <p class="font-medium text-gray-600">
                                                        {{ __('Discount') }}
                                                        <span>
                                                            -<span x-text="money(discount)"></span>
                                                        </span>
                                                    </p>
                                                </template>

                                                <template x-if="shipping > 0">
                                                    <p class="font-medium text-gray-600">
                                                        {{ __('Shipping Fee') }}
                                                        <span>
                                                            +<span x-text="money(shipping)"></span>
                                                        </span>
                                                    </p>
                                                </template>

                                                <template x-if="discount <= 0 && shipping <= 0">
                                                    <span class="text-gray-400">
                                                        -
                                                    </span>
                                                </template>
                                            </div>
                                        </div>

                                        {{-- Payment / Release --}}
                                        <div class="w-[240px] shrink-0 rounded-xl border border-emerald-100 bg-emerald-50/60 px-4 py-3 text-right">

                                            <div class="flex items-center justify-end gap-2">
                                                <span class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">
                                                    Payment
                                                </span>

                                                @if ($isCash)
                                                    <span class="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold uppercase text-emerald-700">
                                                        Cash
                                                    </span>
                                                @else
                                                    <span class="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-semibold uppercase text-gray-600">
                                                        {{ $purchaseItem->disbursement_type_name }}
                                                    </span>
                                                @endif
                                            </div>

                                            {{-- IMPORTANT:
                                                release_total is the server-side
                                                authoritative amount.
                                            --}}
                                            <div class="mt-1 whitespace-nowrap text-2xl font-extrabold leading-tight tracking-tight text-gray-900">
                                                ₱{{ number_format($workflowItem->release_total, 2) }}
                                            </div>

                                            @if ($isCash)
                                                <span class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">
                                                    (Origin: ₱<span x-text="money(finalAmount)"></span>)
                                                </span>
                                            @else
                                                <div class="mt-1 break-words text-xs text-gray-500">
                                                    {{ $purchaseItem->payment_details }}
                                                </div>
                                            @endif
                                        </div>
                                    </div>

                                    {{-- Actions --}}
                                    <div class="flex shrink-0 items-center gap-2">
                                        @if (!$isCash)
                                            <x-approve-button
                                                type="button"
                                                x-on:click="$dispatch('purchase-modal', {
                                                    workflowItemId: {{ $workflowItem->id }}
                                                })"
                                                wire:loading.attr="disabled"
                                            >
                                                {{ __('Release') }}
                                            </x-approve-button>
                                        @endif
                                    </div>
                                </div>

                                {{-- =================================================
                                    MOBILE
                                ================================================== --}}
                                <div class="md:hidden">

                                    {{-- Item Header --}}
                                    <div class="flex items-start gap-3">

                                        <div class="h-14 w-14 shrink-0 overflow-hidden rounded-md bg-gray-100">
                                            <img
                                                src="{{ $item?->primaryImage
                                                    ? Storage::url($item->primaryImage->path)
                                                    : asset('images/default-item.png') }}"
                                                alt="{{ $purchaseItem->item_name }}"
                                                class="h-full w-full object-cover"
                                            >
                                        </div>

                                        <div class="min-w-0 flex-1">
                                            <button
                                                x-on:click="$dispatch('open-item', {
                                                    itemId: {{ $purchaseItem->item_id }}
                                                })"
                                                class="block w-full truncate text-left text-sm font-semibold text-gray-900"
                                                wire:loading.attr="disabled"
                                            >
                                                {{ $purchaseItem->item_name }}
                                            </button>

                                            <div class="mt-1 truncate text-xs text-gray-500">
                                                {{ $purchaseItem->sku }}
                                            </div>
                                        </div>
                                    </div>

                                    {{-- Item Details --}}
                                    <div class="mt-3 rounded-lg bg-gray-50 p-3">

                                        <div class="grid grid-cols-2 gap-4">

                                            <div class="min-w-0">
                                                <div class="text-[10px] font-semibold uppercase tracking-wide text-gray-400">
                                                    Qty
                                                </div>

                                                <div class="mt-1 text-sm font-semibold text-gray-900">
                                                    {{ number_format($purchaseItem->quantity) }}
                                                </div>
                                            </div>

                                            <div class="min-w-0">
                                                <div class="text-[10px] font-semibold uppercase tracking-wide text-gray-400">
                                                    Unit Price
                                                </div>

                                                <div class="mt-1 whitespace-nowrap text-sm font-semibold text-gray-900">
                                                    ₱{{ number_format($purchaseItem->unit_price, 2) }}
                                                </div>
                                            </div>
                                        </div>

                                        {{-- Adjustments --}}
                                        <div class="mt-3 border-t border-gray-200 pt-3">
                                            <div class="space-y-1">

                                                <template x-if="discount > 0">
                                                    <div class="flex items-center justify-between text-xs">
                                                        <span class="text-gray-500">
                                                            {{ __('Discount') }}
                                                        </span>

                                                        <span class="font-medium text-gray-700">
                                                            -₱<span x-text="money(discount)"></span>
                                                        </span>
                                                    </div>
                                                </template>

                                                <template x-if="shipping > 0">
                                                    <div class="flex items-center justify-between text-xs">
                                                        <span class="text-gray-500">
                                                            {{ __('Shipping Fee') }}
                                                        </span>

                                                        <span class="font-medium text-gray-700">
                                                            +₱<span x-text="money(shipping)"></span>
                                                        </span>
                                                    </div>
                                                </template>

                                            </div>
                                        </div>

                                        {{-- Amount --}}
                                        <div class="mt-3 border-t border-gray-200 pt-3">
                                            <div class="flex items-center justify-between gap-3">

                                                <div>
                                                    <div class="text-[10px] font-semibold uppercase tracking-wide text-emerald-600">
                                                        Amount to Release
                                                    </div>

                                                    @if ($isCash)
                                                        <div class="mt-0.5 text-[10px] text-gray-400">
                                                            Origin:
                                                            ₱<span x-text="money(finalAmount)"></span>
                                                        </div>
                                                    @endif
                                                </div>

                                                {{-- FIX:
                                                     use release_total, not purchaseItem->amount
                                                --}}
                                                <div class="whitespace-nowrap text-xl font-extrabold leading-tight tracking-tight text-emerald-800">
                                                    ₱{{ number_format($workflowItem->release_total, 2) }}
                                                </div>
                                            </div>
                                        </div>
                                    </div>

                                    {{-- Payment Type --}}
                                    <div class="mt-3 flex items-center justify-between">
                                        <div class="text-[10px] font-semibold uppercase tracking-wide text-gray-400">
                                            Payment
                                        </div>

                                        @if ($isCash)
                                            <span class="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-bold uppercase text-emerald-700">
                                                Cash
                                            </span>
                                        @else
                                            <span class="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] font-semibold uppercase text-gray-600">
                                                {{ $purchaseItem->disbursement_type_name }}
                                            </span>
                                        @endif
                                    </div>

                                    {{-- Vendor --}}
                                    <div class="mt-3">
                                        <div class="text-[10px] uppercase tracking-wide text-gray-400">
                                            Vendor
                                        </div>

                                        @if ($itemVendor)
                                            <button
                                                x-on:click="$dispatch('open-vendor', {
                                                    vendorId: {{ $itemVendor->vendor_id }}
                                                })"
                                                class="mt-0.5 block max-w-full truncate text-left text-sm font-medium text-gray-700"
                                                wire:loading.attr="disabled"
                                            >
                                                {{ $purchaseItem->vendor_name }}
                                            </button>
                                        @else
                                            <div class="mt-0.5 text-sm text-gray-500">
                                                {{ $purchaseItem->vendor_name ?: '-' }}
                                            </div>
                                        @endif
                                    </div>

                                    {{-- Mobile Actions --}}
                                    <div class="mt-4">
                                        @if (!$isCash)
                                            <x-approve-button
                                                type="button"
                                                x-on:click="$dispatch('purchase-modal', {
                                                    workflowItemId: {{ $workflowItem->id }}
                                                })"
                                                wire:loading.attr="disabled"
                                                class="w-full"
                                            >
                                                {{ __('Release') }}
                                            </x-approve-button>
                                        @endif
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    </div>

                    {{-- =====================================================
                        Footer
                    ====================================================== --}}
                    <div class="border-t border-gray-100 bg-gray-50 px-4 py-4 sm:px-5">

                        {{-- Desktop Footer --}}
                        <div class="hidden items-center justify-end gap-4 sm:flex">

                            <div class="text-right">
                                <div class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">
                                    Total Amount to Release
                                </div>

                                <div class="mt-0.5 whitespace-nowrap text-3xl font-extrabold leading-none tracking-tight text-gray-900">
                                    ₱{{ number_format($request->account_total, 2) }}
                                </div>
                            </div>

                            @if ($hasCash)
                                <x-approve-button
                                    type="button"
                                    x-on:click="$dispatch('cash-handover-modal', {
                                        workflowId: {{ $request->account_workflow->id }}
                                    })"
                                    wire:loading.attr="disabled"
                                >
                                    {{ __('Released Cash All') }}
                                </x-approve-button>
                            @endif
                        </div>

                        {{-- Mobile Footer --}}
                        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:hidden">

                            <div class="min-w-0 rounded-lg bg-emerald-50 p-3">
                                <div class="text-[10px] font-semibold uppercase tracking-wide text-emerald-600">
                                    Total Amount to Release
                                </div>

                                <div class="mt-1 whitespace-nowrap text-xl font-extrabold leading-tight tracking-tight text-emerald-800">
                                    ₱{{ number_format($request->account_total, 2) }}
                                </div>
                            </div>

                            @if ($hasCash)
                                <div class="mt-4">
                                    <button
                                        type="button"
                                        x-on:click="$dispatch('cash-handover-modal', {
                                            workflowId: {{ $request->account_workflow->id }}
                                        })"
                                        wire:loading.attr="disabled"
                                        class="inline-flex w-full items-center justify-center gap-2 rounded-lg
                                            border border-emerald-300 bg-emerald-600 px-4 py-3
                                            text-sm font-bold text-white shadow-sm
                                            transition hover:bg-emerald-700
                                            active:scale-[0.98]
                                            focus:outline-none focus:ring-2 focus:ring-emerald-500/30
                                            disabled:cursor-not-allowed disabled:opacity-50"
                                    >
                                        {{ __('Release Cash All') }}
                                    </button>
                                </div>
                            @endif
                        </div>
                    </div>
                </div>
            @endforeach

            <div class="mt-6">
                {{ $requests->links() }}
            </div>
        @else
            <div class="rounded-lg border border-dashed border-gray-300 bg-white px-6 py-12 text-center">
                <div class="text-sm font-medium text-gray-900">
                    No pending budget requests.
                </div>

                <div class="mt-1 text-sm text-gray-500">
                    There are currently no purchase requests waiting for budget.
                </div>
            </div>
        @endif
    </div>

    <livewire:accountings.modals.purchase-item />
    <livewire:accountings.modals.release-cash />
</div>